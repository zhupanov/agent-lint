...
|-------|-------------|---------|
| `version` | Version of agent-lint (e.g., `version: "4.0.2"`) | Latest release |
...
`version: "4.0.2"`), no API call is made and the token is never used.
...
> `zhupanov/agent-lint@v4` with `path: "."` and `version: "4.0.2"`. Add it
> to the existing CI workflow if one exists, otherwise create
> `.github/workflows/ci.yaml` with `permissions: contents: read`.**
>
> **Pin to an exact version** (e.g., `version: "4.0.2"`) to protect your
