...
| S030 | `orphaned-skill-files` | Files anywhere in a skill `scripts/` tree not owned by skill-local `.md` (exact relative path or exact unique basename, including normal Markdown delimiters and sentence punctuation), transitive reachability from a Markdown-live supported harness script, or a bounded repository Python-test fixture reference. Python ownership applies only below `scripts/fixtures/` through a literal repository-relative path, a literal fixture-directory child path, or a literal `Path.glob` enumeration (including an explicit literal stem-suffix companion); it never owns runtime scripts. Repository-ignored paths and conventional cache artifacts (`__pycache__`, `*.pyc`, …) are excluded from discovery | Always | error |
